...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert pinned safetensors to TLIEWGT FP16")
...
def tensors_from_safetensors(path: Path, names: list[str]) -> Iterator[tuple[str, NDArray[Any]]]:
    with safe_open(path, framework="pt", device="cpu") as source:  # type: ignore[no-untyped-call]
        for name in names:
            ...
            yield name, tensor.half().contiguous().numpy()
...
def main() -> None:
    ...
    output = args.output or args.model_dir / "model-fp16.tliewgt"
    ...
    converted = manifest["cuda_converted_format"]
    ...
    if sidecar["file_sha256"] != expected_file_sha256 or sidecar["file_size"] != expected_file_size:
        ...
        raise ValueError(
            "converted FP16 TLIEWGT does not match the pinned deterministic size and SHA-256"
        )
